        ...
        <div class="mt-10 p-6 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700">
          <form id="paypal-form" action="https://www.paypal.com/cgi-bin/webscr" method="post" target="_blank">
            ...
            <div class="text-left mb-4">
              ...
              <!-- ADJUSTED PREVIEW: Smaller fonts (text-xs/text-sm) and tighter padding to prevent cutoff -->
              <div class="mt-3 px-3 py-2.5 bg-blue-50 dark:bg-primary/10 rounded-lg border-2 border-dashed border-primary/40 flex items-center gap-2 overflow-hidden shadow-sm">
                <div class="bg-primary rounded-full p-1 flex-shrink-0">
                  <Icon name="tabler:world" class="w-3.5 h-3.5 text-white" />
                </div>
                <span class="text-xs md:text-sm font-mono whitespace-nowrap leading-none">
                    <span class="text-slate-500 dark:text-slate-400">https://</span><span id="vanity-preview" class="text-primary dark:text-blue-400 font-bold underline decoration-accent/30 decoration-2 underline-offset-2">yourstudio</span><span class="text-slate-500 dark:text-slate-400">.address.makeup</span>
                </span>
              </div>
            </div>
